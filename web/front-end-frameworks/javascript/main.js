...
(function ($) {
                                                ...
                                                $.ajax("http://localhost:8080/Java-Matlab-Integration/IonImageGeneratorServlet/generate-image",
                                                        {
                                                            ...
                                                            "success": function (data) {
                                                                ...
                                                                drawImage(config, range, data.sort(), result, dimensions[0]);
                                                            },
                                                            "error": function (xhr) {
                                                                errorCallback(xhr.statusText);
                                                            }
    ...
    function drawImage(config, range, bounds, dataArray, dimension) {
        ...
        for (var i = 0; i < dimension; i++) {
            ...
            /*var interval = Math.floor(bounds[bounds.length-1] / 50);
            var domain = [];
            for (var k = 0; k < 49; k++)
                domain.push(k*interval);
            domain.push(bounds[bounds.length-1]);
            var colors = [];
            for (var j = 0; j < 50; j++) {
            ...
            colors = colors.split(",").map(e => "#" + e);
            var colorScale = d3.scaleQuantize()
                    .domain([0, d3.max(bounds)])
                    .range(colors);
